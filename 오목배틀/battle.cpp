/*
 * battle.cpp  —  head-to-head: my AI (2023184027) vs friend (2023184014)
 *   Drives both AIs through their public functions exactly like the judge,
 *   referees with the exact rules (5=win, 6=loss, black 3-3/4-4 loss).
 *   compile: cl /EHsc /O2 /utf-8 battle.cpp
 */
#include <cstdio>
#include <cstring>
#include <cstdlib>
#include "Omock_2023184027.h"   /* my AI + ec_* referee + EC_* */
#include "2023184014.h"          /* friend AI */
...
static void reset_all() {
    ...
    g_init_2023184027 = 1;
    memset(board_2023184014, 0, sizeof(board_2023184014));
}

/* referee for a just-placed stone at (r=row,c=col): 0 cont, 1 win, 2 foul/overline */
...
static int play_one(int myColor, int* why) {
    ...
    *why = 0;
    ...
    for (int ply = 0; ply < EC_SZ * EC_SZ; ply++) {
        ...
        if (turn == EC_BLACK) {
            if (mine) BlackAttack_2023184027(&x, &y); else BlackAttack_2023184014(&x, &y);
        } else {
            if (mine) WhiteAttack_2023184027(&x, &y); else WhiteAttack_2023184014(&x, &y);
        }
        /* illegal -> mover loses */
        ...
        /* notify opponent's defence (judge order) */
        if (turn == EC_BLACK) {
            if (mine) WhiteDefence_2023184014(x, y); else WhiteDefence_2023184027(x, y);
        } else {
            if (mine) BlackDefence_2023184014(x, y); else BlackDefence_2023184027(x, y);
        }
...
int main(int argc, char** argv) {
    ...
    printf("=== BATTLE: MINE(2023184027) vs FRIEND(2023184014), %d games each color ===\n", games);
